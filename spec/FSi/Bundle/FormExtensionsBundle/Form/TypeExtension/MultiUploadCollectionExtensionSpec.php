...
class MultiUploadCollectionExtensionSpec extends ObjectBehavior
{
    ...
    {
        $resolver->setDefault('multi_upload_field', null)->shouldBeCalled()->willReturn($resolver);
        $resolver->setAllowedTypes('multi_upload_field', ['null', 'string'])->shouldBeCalled()->willReturn($resolver);
    ...
    {
        $builder
            ->addEventSubscriber(Argument::type(MultiUploadCollectionListener::class))
            ->shouldBeCalled()
            ->willReturn($builder)
        ;
